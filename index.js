...
async function runScan() {
  ...
  for (const pick of topPicks) {
    ...
    if (qty < 1) continue;
    await placeOrder({ symbol: pick.ticker, qty, side: 'buy', stopLoss: pick.price*(1-STOP_LOSS_PCT), takeProfit: pick.price*(1+TAKE_PROFIT_PCT) });
  }
}
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  #lock-screen h1 { font-size: 28px; color: #00ff88; }
  #lock-screen p { color: #666; font-size: 14px; }
  #pin-input { background: #1a1a1a; border: 1px solid #333; border-radius: 12px; padding: 14px 20px; color: #fff; font-size: 18px; text-align: center; width: 220px; outline: none; letter-spacing: 4px; }
  #pin-btn { background: #00ff88; color: #000; border: none; border-radius: 12px; padding: 14px 40px; font-weight: 700; font-size: 16px; cursor: pointer; width: 220px; }
...
<script>
  ...
  function addMsg(text, type) {
    ...
    if (type === 'bot') div.innerHTML = '<div class="bot-name">PULSETRADER</div>' + text;
...
app.listen(PORT, async () => {
  ...
  try { const a = await getAccount(); console.log(`✅ Alpaca — equity: $${a.equity}`); }
